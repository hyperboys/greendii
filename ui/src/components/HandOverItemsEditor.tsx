...
export default function HandOverItemsEditor({
  ...
  // Swaps the whole item (desc/qty/unit/note/detailRows/images) with its neighbor.
              ...
              {items.map((item, index) => (
                <Fragment key={`${item.seq ?? index}-${index}`}>
                  ...
                  <tr className={`border-t align-top ${activeItemIdx === index ? 'border-green-400 bg-green-50/40' : 'border-gray-100'}`}>
                    <td className="px-2 py-2.5 pt-3.5 text-xs text-gray-400">
                      <div className="flex items-center gap-1">
                        <span>{index + 1}</span>
                        <div className="flex flex-col">
                          <button
                            type="button"
                            className="p-0.5 text-gray-400 transition-colors hover:text-green-700 disabled:cursor-not-allowed disabled:opacity-30"
                            disabled={index === 0}
                            onClick={() => moveItem(index, 'up')}
                            title="ย้ายรายการนี้ขึ้น"
                          >
                            <ChevronUp size={13} />
                          </button>
                          <button
